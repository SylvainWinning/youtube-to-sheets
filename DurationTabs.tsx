...
export function DurationTabs({ selectedTab, onTabChange, videos }: DurationTabsProps) {
  const tabCounts = React.useMemo(() => SHEET_TABS.map(tab => getVideoCountByDuration(videos, tab)), [videos]);
  ...
  return (
    // Reduced bottom margin from mb-6 to mb-2 to minimize gray area
    <div className="mb-2">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
        <div className="flex col-span-2 sm:col-span-1">
          ...
          <ShuffleButton onClick={() => handleShuffle(-1)} disabled={videos.length === 0} />
        </div>
        ...
        {SHEET_TABS.map((tab, index) => (
          <div key={tab.name} className="flex">
            ...
            <ShuffleButton onClick={() => handleShuffle(index)} disabled={tabCounts[index] === 0} />
          </div>
        ))}
      </div>
    </div>
